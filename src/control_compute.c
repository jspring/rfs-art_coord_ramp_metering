...
//		occ_out_prime = occ_in * IN_LANES/OUT_LANES + 100 * VEHICLE_EFFECTIVE_LENGTH * q_r / SHOCH_WAVE_SPEED / OUT_LANES ;
	...
	if(get_current_time()<PLANTIME1)
	{
		if(new_rate<METER_LOWER_BOUND1)
		{
			new_rate=METER_LOWER_BOUND1;
		}
	}
	else
	{
		if(get_current_time()<PLANTIME2)
		{
			if(new_rate<METER_LOWER_BOUND2)
			{
				new_rate=METER_LOWER_BOUND2;
			}
		}
		else
		{
			if(new_rate<METER_LOWER_BOUND3)
			{
				new_rate=METER_LOWER_BOUND3;
			}
		}
	}
